...
import de.staticcode.candy.module.modules.COMBAT.AutoSword;
...
public class C02PacketUseEntity implements Packet< INetHandlerPlayServer > {
    ...
    public C02PacketUseEntity ( Entity entity , C02PacketUseEntity.Action action ) {
        if (Module.getByName ( "AutoSword" ).isToggled ( )) {
            int itemSlot = AutoSword.findItem ( );
            if (itemSlot != -1)
                Minecraft.thePlayer.inventory.currentItem = itemSlot;
        }
