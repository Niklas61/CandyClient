...
public class GuiPlayerTabOverlay extends Gui {
	...
	public void renderPlayerlist(int width, Scoreboard scoreboardIn, ScoreObjective scoreObjectiveIn) {
		...
		for (NetworkPlayerInfo networkplayerinfo : list) {
			...
			if (AntiNick.Usernames.containsKey(s1)) {
				s1 = this.getPlayerName(networkplayerinfo) + " �2(�7" + AntiNick.Usernames.get(s1) + "�2)";
			}
			...
			if (scoreObjectiveIn != null
				...
				k = Minecraft.fontRendererObj.getStringWidth(" " + scoreboardIn
		...
		if (list1 != null) {
			drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1,
					k1 + list1.size() * Minecraft.fontRendererObj.FONT_HEIGHT, Integer.MIN_VALUE);
		...
		for (int k4 = 0; k4 < l3; ++k4) {
			...
			if (k4 < list.size()) {
				...
				if (AntiNick.Usernames.containsKey(s1)) {
					s1 = this.getPlayerName(networkplayerinfo1) + " �2(�7" + AntiNick.Usernames.get(s1) + "�2)";
				}
		...
		if (list2 != null) {
			k1 = k1 + i4 * 9 + 1;
			drawRect(width / 2 - l1 / 2 - 1, k1 - 1, width / 2 + l1 / 2 + 1,
					k1 + list2.size() * Minecraft.fontRendererObj.FONT_HEIGHT, Integer.MIN_VALUE);
		...
		if (p_175247_1_.getRenderType() == IScoreObjectiveCriteria.EnumRenderType.HEARTS) {
			...
			if (j > 0) {
				...
				if (f > 3.0F) {
					...
					Minecraft.fontRendererObj.drawStringWithShadow(s,
							(float) ((p_175247_5_ + p_175247_4_) / 2 - Minecraft.fontRendererObj.getStringWidth(s) / 2),
							(float) p_175247_2_, i1);
				}
			}
		} else {
			String s1 = EnumChatFormatting.YELLOW + "" + i;
			Minecraft.fontRendererObj.drawStringWithShadow(s1,
					(float) (p_175247_5_ - Minecraft.fontRendererObj.getStringWidth(s1)), (float) p_175247_2_, 16777215);
		}
	}
