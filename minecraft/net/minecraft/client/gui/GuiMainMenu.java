...
import de.staticcode.candy.alt.AccountManager;
...
public class GuiMainMenu extends GuiScreen implements GuiYesNoCallback {
    ...
    public void initGui ( ) {
        ...
        // + 72 + 12));
        this.buttonList.add ( new GuiButton ( 99 , this.width / 2 - 100 , j + 36 + 12 , 200 , 20 , "Alt Login" ) );
    ...
    private void addSingleplayerMultiplayerButtons ( int p_73969_1_ , int p_73969_2_ ) {
        ...
        // this.buttonList.add(this.realmsButton = new GuiButton(14, this.width
        // / 2 - 100, p_73969_1_ + p_73969_2_ * 2,
    ...
    protected void actionPerformed ( GuiButton button ) throws IOException {
        ...
        if (button.id == 99) {
            mc.displayGuiScreen ( new AccountManager ( this ) );
        }
    ...
    private void rotateAndBlurSkybox ( float p_73968_1_ ) {
        ...
        for ( int j = 0; j < i; ++j ) {
            ...
            worldrenderer.pos ( k , l , this.zLevel ).tex ( 0.0F + f1 , 1.0D )
                    .color ( 1.0F , 1.0F , 1.0F , f ).endVertex ( );
            worldrenderer.pos ( k , 0.0D , this.zLevel ).tex ( 1.0F + f1 , 1.0D )
                    .color ( 1.0F , 1.0F , 1.0F , f ).endVertex ( );
            worldrenderer.pos ( 0.0D , 0.0D , this.zLevel ).tex ( 1.0F + f1 , 0.0D )
                    .color ( 1.0F , 1.0F , 1.0F , f ).endVertex ( );
            worldrenderer.pos ( 0.0D , l , this.zLevel ).tex ( 0.0F + f1 , 0.0D )
                    .color ( 1.0F , 1.0F , 1.0F , f ).endVertex ( );
        }
    ...
    private final ResourceLocation rs = new ResourceLocation ( "textures/gui/Background.png" );

    private void renderRadium ( int mouseX , int mouseY ) {

        this.mc.getTextureManager ( ).bindTexture ( rs );
        drawScaledCustomSizeModalRect ( 0 , 0 ,
                ( float ) -( ( ( double ) ScaledResolution.getScaledWidth ( ) / 2d
                        + ( double ) ScaledResolution.getScaledWidth ( ) ) ) / 1000d ,
                ( float ) ( -( ( double ) ScaledResolution.getScaledHeight ( ) / 2d
                        + ( double ) ScaledResolution.getScaledHeight ( ) ) ) / 1000d ,
                width , height , width , height , width , height );

        for ( Ball balls : Ball.instances ) {
            for ( Ball b : Ball.instances ) {
                if (balls.distance ( mouseX , mouseY ) <= 50 && b.distance ( mouseX , mouseY ) <= 50) {
                    balls.drawLineTo ( b );
                }
            }
            balls.render ( );
        ...
        GL11.glPushMatrix ( );
        if (Module.getByName ( "MainMenuScale" ).isToggled ( )) {
            GL11.glScaled ( GuiComponent.getByName ( "Main Scale" ).getCurrent ( ) ,
                    GuiComponent.getByName ( "Main Scale" ).getCurrent ( ) ,
                    GuiComponent.getByName ( "Main Scale" ).getCurrent ( ) );
            this.drawString ( Minecraft.fontRendererObj , "§3" + Candy.NAME ,
                    ( int ) ( this.width / 2 / GuiComponent.getByName ( "Main Scale" ).getCurrent ( ) )
                            - Minecraft.fontRendererObj.getStringWidth ( Candy.NAME ) / 2 ,
                    ( int ) ( this.height / 6 / GuiComponent.getByName ( "Main Scale" ).getCurrent ( ) ) - 5 , -1 );
        } else {
    ...
    public void drawScreen ( int mouseX , int mouseY , float partialTicks ) {
        if (FontLoader.fontEffect) {
            ...
            this.renderRadium ( mouseX , mouseY );
            GlStateManager.pushMatrix ( );
            ...
            // -256);
            GlStateManager.popMatrix ( );
            String s = Candy.NAME + " " + Candy.VERSION;
            ...
            String s1 = "Developed by StaticCode";
            String s2 = "(Violence remade)";
    ...
    public static class GuiPatch extends GuiScreen {
        public GuiPatch ( ) {
        }
        ...
        public void drawScreen ( int mouseX , int mouseY , float partialTicks ) {
            ...
            drawRect ( 0 , 0 , ScaledResolution.getScaledWidth ( ) , ScaledResolution.getScaledHeight ( ) , Candy.RGBtoHEX ( 255 , 255 , 255 , ( int ) this.alpha ) );
            if (this.alpha <= 1) {
                mc.displayGuiScreen ( new GuiMainMenu ( ) );
                FontLoader.fontEffect = true;
            }
            super.drawScreen ( mouseX , mouseY , partialTicks );
        }
    }
