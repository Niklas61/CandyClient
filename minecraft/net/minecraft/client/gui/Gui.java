...
public class Gui
{
     ...
     */
    public static void drawScaledCustomSizeModalRect(int x, int y, double d, double e, int uWidth, int vHeight, int width, int height, float tileWidth, float tileHeight)
    {
        ...
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos( x , y + height , 0.0D).tex( d * f , (e + (float)vHeight) * f1 ).endVertex();
        worldrenderer.pos( x + width , y + height , 0.0D).tex( (d + (float)uWidth) * f , (e + (float)vHeight) * f1 ).endVertex();
        worldrenderer.pos( x + width , y , 0.0D).tex( (d + (float)uWidth) * f , e * f1 ).endVertex();
        worldrenderer.pos( x , y , 0.0D).tex( d * f , e * f1 ).endVertex();
        tessellator.draw();
    }
}
