...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private void setupViewBobbing ( float partialTicks ) {
        if (this.mc.getRenderViewEntity ( ) instanceof EntityPlayer) {
            ...
            GlStateManager.rotate ( MathHelper.sin ( f1 * ( float ) Math.PI ) * f2 * 10.0F , 0.0F , 0.0F , 1.0F );
            GlStateManager.rotate ( Math.abs ( MathHelper.cos ( f1 * ( float ) Math.PI - 0.2F ) * f2 ) * 10.0F , 1.0F , 0.0F , 0.0F );
    ...
    private void setupCameraTransform ( float partialTicks , int pass ) {
        ...
        float f1 = Minecraft.thePlayer.prevTimeInPortal
    ...
    private void renderWorldDirections ( float partialTicks ) {
        if (this.mc.gameSettings.showDebugInfo && !this.mc.gameSettings.hideGUI && !Minecraft.thePlayer.hasReducedDebug ( )
    ...
    private void renderWorldPass ( int pass , float partialTicks , long finishTimeNano ) {
        ...
        if (!this.debugView) {
            ...
            try {
                if (RenderUtils.shouldRender) {
                    GlStateManager.disableTexture2D ( );
                    GlStateManager.disableDepth ( );
                    for ( BlockPos blockPos : RenderUtils.renderList.keySet ( ) ) {
                        int id = RenderUtils.renderList.get ( blockPos );
                        if (id == -1) {
                            id = Block.getIdFromBlock ( mc.theWorld.getBlockState ( blockPos ).getBlock ( ) );
                        }
                        if (mc.theWorld.getBlockState ( blockPos ).getBlock ( ) == Block
                                .getBlockById ( id )) {
                            GlStateManager.color ( 0.0F ,
                                    0.4F ,
                                    ( float ) ( ( float ) Math.sin ( ( double ) System.currentTimeMillis ( ) / 200d ) / 5d + 0.8F ) ,
                                    ( float ) ( ( float ) Math.sin ( ( double ) System.currentTimeMillis ( ) / 500d ) / 5d + 0.95F ) );
                            RenderGlobal.func_181561_a ( Minecraft.getMinecraft ( ).theWorld.getBlockState ( blockPos )
                                    .getBlock ( ).getSelectedBoundingBox ( Minecraft.getMinecraft ( ).theWorld , blockPos )
                                    .expand ( 0.090000000949949026D , 0.090000000949949026D , 0.090000000949949026D )
                                    .offset ( -d0 , -d1 , -d2 ) );
                        }
                    }
                    GlStateManager.enableDepth ( );
