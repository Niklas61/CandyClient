...
public abstract class RendererLivingEntity< T extends EntityLivingBase > extends Render< T > {
    ...
    public void doRender ( T entity , double x , double y , double z , float entityYaw , float partialTicks ) {
        ...
        try {
            ...
            float prevYaw = entity.prevRenderYawOffset;
            float renderYaw = entity.renderYawOffset;
            float prevRotationYaw = entity.prevRotationYawHead;
            float pitch = entity.rotationPitch;
            float prevRotPitch = entity.prevRotationPitch;

            if (entity instanceof EntityPlayer && entity.equals ( Minecraft.thePlayer )) {
                yaw = RotationUtils.server_yaw;
                ...
                if (Killaura.underAttack != null || Module.getByName ( "Scaffold" ).isToggled ( )
                    ...
                    pitch = RotationUtils.server_pitch;
                    prevRotPitch = RotationUtils.server_pitch;
                }
            } else
                prevRotationYaw = entity.prevRotationYaw;


            float f = this.interpolateRotation ( prevYaw , renderYaw , partialTicks );
            float f1 = this.interpolateRotation ( prevRotationYaw , yaw , partialTicks );
            float f2 = f1 - f;

            if (entity.isRiding ( ) && entity.ridingEntity instanceof EntityLivingBase) {
                EntityLivingBase entitylivingbase = ( EntityLivingBase ) entity.ridingEntity;

                f = this.interpolateRotation ( prevYaw , renderYaw , partialTicks );
                f2 = f1 - f;
            ...
            float f7 = pitch + ( pitch - prevRotPitch ) * partialTicks;
    ...
    protected boolean setScoreTeamColor ( T entityLivingBaseIn ) {
        ...
        if (entityLivingBaseIn.hurtTimeNoCam <= 0)
            GlStateManager.color ( 0.0F , 0.7F , 0.9F );
        else
    ...
    protected void renderModel ( T entitylivingbaseIn , float p_77036_2_ , float p_77036_3_ , float p_77036_4_ ,
        ...
        boolean flag1 = !flag && !entitylivingbaseIn.isInvisibleToPlayer ( Minecraft.thePlayer );

        if (flag || flag1 || Module.getByName ( "NoInvisibles" ).isToggled ( )) {
    ...
    public void renderName ( T entity , double x , double y , double z ) {
        if (this.canRenderName ( entity )) {
            ...
            if (d0 < ( double ) ( f * f )) {
                ...
                if (Module.getByName ( "Nametags" ).isToggled ( )) {

                    if (FriendManager.friends.contains ( s )) {
                        s = "§2" + entity.getDisplayName ( ).getFormattedText ( );
                    }
                    ...
                    GlStateManager.scale ( -0.05666667F , -0.05666667F , 0.05666667F );
                    GlStateManager.disableLighting ( );
                    GlStateManager.depthMask ( false );
                    GlStateManager.enableBlend ( );
                    GlStateManager.disableTexture2D ( );
                    ...
                    worldrenderer.pos ( -i - 1 , -1.0D , 0.0D ).color ( 0.0F , 0.0F , 0.0F , 0.6F ).endVertex ( );
                    worldrenderer.pos ( -i - 1 , 8.0D , 0.0D ).color ( 0.0F , 0.0F , 0.0F , 0.6F ).endVertex ( );
                    worldrenderer.pos ( i + 1 , 8.0D , 0.0D ).color ( 0.0F , 0.0F , 0.0F , 0.6F ).endVertex ( );
                    worldrenderer.pos ( i + 1 , -1.0D , 0.0D ).color ( 0.0F , 0.0F , 0.0F , 0.6F ).endVertex ( );
                    tessellator.draw ( );
                    GlStateManager.enableTexture2D ( );
