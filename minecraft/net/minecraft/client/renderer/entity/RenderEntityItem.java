...
public class RenderEntityItem extends Render< EntityItem > {
    ...
    private int func_177077_a ( EntityItem itemIn , double p_177077_2_ , double p_177077_4_ , double p_177077_6_ ,
        ...
        if (item == null) {
            ...
            if (!Module.getByName ( "ItemPhysics" ).isToggled ( ))
                GlStateManager.translate ( ( float ) p_177077_2_ , ( float ) p_177077_4_ + f1 + 0.25F * f2 ,
                        ( float ) p_177077_6_ );
            else
                GlStateManager.translate ( ( float ) p_177077_2_ , ( float ) p_177077_4_ + 0.25F , ( float ) p_177077_6_ );

            if (( flag || this.renderManager.options != null ) && !Module.getByName ( "ItemPhysics" ).isToggled ( )) {
            ...
            if (Module.getByName ( "ItemPhysics" ).isToggled ( )) {
                ...
                if (!itemIn.onGround)
                    itemIn.toRotate += 1.0F;

            } else {
                if (itemIn.toRotate != 0.0F)
                    itemIn.toRotate = 0.0F;
            }

            if (itemIn.toRotate > 0.0F) {
                if (!itemIn.onGround) {
                    GlStateManager.rotate ( itemIn.toRotate , GuiComponent.getByName ( "Rotate X" ).isToggled ( ) ? 360F : 0F ,
                            GuiComponent.getByName ( "Rotate Y" ).isToggled ( ) ? 360F : 0F ,
                            GuiComponent.getByName ( "Rotate Z" ).isToggled ( ) ? 360F : 0F );
                } else {
    ...
    public void doRender ( EntityItem entity , double x , double y , double z , float entityYaw , float partialTicks ) {
        ...
        if (Module.getByName ( "ItemESP" ).isToggled ( )) {
            ...
            GL11.glColor4d ( ( float ) Math.sin ( ( double ) System.currentTimeMillis ( ) / 100D ) / 5d + 0.1D , 0.6D , 0.7D , ( float ) ( ( float ) Math.sin ( ( double ) System.currentTimeMillis ( ) / 400D ) / 5d + 0.5F ) );
            RenderGlobal.func_181561_a (
                    new AxisAlignedBB ( xPos - 0.2d , yPos , zPos - 0.2d , xPos + 0.2d , yPos + 0.4d , zPos + 0.2d ) );
