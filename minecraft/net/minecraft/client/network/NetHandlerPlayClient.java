...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    public void handleEntityVelocity ( S12PacketEntityVelocity packetIn ) {
        ...
        if (entity != null) {
            if (Module.getByName ( "NoVelocity" ).isToggled ( ) && entity == Minecraft.thePlayer) {

                if (GuiComponent.getByName ( "AAC4" ).isToggled ( )) {


                    Minecraft.getMinecraft ( ).gameSettings.keyBindSprint.pressed = true;
                    Minecraft.thePlayer.sprintToggleTimer = 0;
                    entity.setVelocity ( ( double ) packetIn.getMotionX ( ) / 7379.0D , ( double ) packetIn.getMotionY ( ) / 8000.0D ,
                            ( double ) packetIn.getMotionZ ( ) / 7379.0D );

                    if (!entity.onGround)
                        entity.motionY -= 0.010423423D;

                    Minecraft.getMinecraft ( ).gameSettings.keyBindSprint.pressed = false;
                    return;
                }

                if (!GuiComponent.getByName ( "Gomme" ).isToggled ( ) && !GuiComponent.getByName ( "Only Jump" ).isToggled ( )) {



                    double x = ( double ) packetIn.getMotionX ( ) / 8000.0D
                            * ( 100d - GuiComponent.getByName ( "Percent" ).getCurrent ( ) ) / 100d;
                    double y = ( double ) packetIn.getMotionY ( ) / 8000.0D
                            * ( 100d - GuiComponent.getByName ( "Percent" ).getCurrent ( ) ) / 100d;
                    double z = ( double ) packetIn.getMotionZ ( ) / 8000.0D
                            * ( 100d - GuiComponent.getByName ( "Percent" ).getCurrent ( ) ) / 100d;

                    entity.setVelocity ( x , y , z );
                    return;
                } else if (GuiComponent.getByName ( "Gomme" ).isToggled ( )) {
                    final double x = ( double ) packetIn.getMotionX ( ) / 8000.0D;
                    final double y = ( double ) packetIn.getMotionY ( ) / 8000.0D;
                    final double z = ( double ) packetIn.getMotionZ ( ) / 8000.0D;

                    final double minilizer = 10f;
                    ...
                    new Thread ( new Runnable ( ) {
                        ...
                        public void run ( ) {
                            ...
                            entity.setVelocity ( x / minilizer , ( y - 0.08f ) / 1.02f , z / minilizer );
                            ...
                            if (Minecraft.getMinecraft ( ).gameSettings.keyBindForward.isKeyDown ( ))
                                Minecraft.move ( Minecraft.thePlayer.rotationYaw , 0.25f ,
                                        ( float ) Minecraft.thePlayer.motionY );
                            else
                                Minecraft.move ( Minecraft.thePlayer.rotationYaw , 0.07f ,
                                        ( float ) Minecraft.thePlayer.motionY );
                ...
                } else if (GuiComponent.getByName ( "Only Jump" ).isToggled ( )) {
                    if (Minecraft.thePlayer.onGround) {
                        Minecraft.thePlayer.jump ( );
                    }

                    entity.setVelocity ( ( double ) packetIn.getMotionX ( ) / 17000.0D ,
                            ( double ) packetIn.getMotionY ( ) / 8000.0D , ( double ) packetIn.getMotionZ ( ) / 17000.0D );
                }
                return;
            }
    ...
    public void handleSpawnPlayer ( S0CPacketSpawnPlayer packetIn ) {
        ...
        // Spawn Player
        List< EntityOtherPlayerMP > destroys = new ArrayList<> ( );
        for ( EntityOtherPlayerMP es : destoryedEntitys )
            destroys.add ( es );

        for ( EntityOtherPlayerMP otherPlayers : destroys ) {
            if (otherPlayers.getGameProfile ( ) == entityotherplayermp.getGameProfile ( )
                    && otherPlayers.getEntityId ( ) == otherPlayers.getEntityId ( ) && otherPlayers.isDead) {

                entityotherplayermp.wasSwinged = entityotherplayermp.wasSwinged;
                entityotherplayermp.wasSprinting = otherPlayers.wasSprinting;
                entityotherplayermp.wasHurting = otherPlayers.wasHurting;

                destoryedEntitys.remove ( otherPlayers );
            }
        }
    ...
    private static final List< EntityOtherPlayerMP > destoryedEntitys = new ArrayList<> ( );

    public void handleDestroyEntities ( S13PacketDestroyEntities packetIn ) {
        PacketThreadUtil.checkThreadAndEnqueue ( packetIn , this , this.gameController );

        for ( int i = 0; i < packetIn.getEntityIDs ( ).length; ++i ) {
            Entity e = this.clientWorldController.getEntityByID ( packetIn.getEntityIDs ( )[ i ] );
            if (e instanceof EntityOtherPlayerMP) {
                destoryedEntitys.add ( ( EntityOtherPlayerMP ) e );
            }
    ...
    public void addToSendQueue ( Packet p_147297_1_ ) {
        ...
        if (Module.getByName ( "Teleport" ).isToggled ( ) && Teleport.current != null) {
            if (p_147297_1_ instanceof C03PacketPlayer.C04PacketPlayerPosition) {
                C04PacketPlayerPosition pos = ( C04PacketPlayerPosition ) p_147297_1_;
                pos.x = Teleport.x;
                pos.y = Teleport.y;
                pos.z = Teleport.z;
                pos.onGround = true;
            } else if (p_147297_1_ instanceof C03PacketPlayer.C06PacketPlayerPosLook) {
                C06PacketPlayerPosLook pos = ( C06PacketPlayerPosLook ) p_147297_1_;
                pos.x = Teleport.x;
                pos.y = Teleport.y;
                pos.z = Teleport.z;
                pos.onGround = true;
            } else if (p_147297_1_ instanceof C03PacketPlayer.C05PacketPlayerLook) {
                return;
            }
        }
    ...
    public void handleAnimation ( S0BPacketAnimation packetIn ) {
        ...
        if (entity != null) {
            if (packetIn.getAnimationType ( ) == 0) {
                ...
                entitylivingbase.wasSwinged = true;
    ...
    public void handleEntityStatus ( S19PacketEntityStatus packetIn ) {
        ...
        if (entity != null) {
            Entity.sendedStatus.add ( entity.getName () );
    ...
    public void handleSetExperience ( S1FPacketSetExperience packetIn ) {
        PacketThreadUtil.checkThreadAndEnqueue ( packetIn , this , this.gameController );
        Minecraft.thePlayer.setXPStats ( packetIn.func_149397_c ( ) , packetIn.getTotalExperience ( ) ,
                packetIn.getLevel ( ) );
    }
    ...
    public void handleStatistics ( S37PacketStatistics packetIn ) {
        ...
        for ( Entry< StatBase, Integer > entry : packetIn.func_148974_c ( ).entrySet ( ) ) {
            ...
            Minecraft.thePlayer.getStatFileWriter ( ).unlockAchievement ( Minecraft.thePlayer , statbase ,
                    i );
        }
    ...
    public void handleCustomPayload ( S3FPacketCustomPayload packetIn ) {
        ...
        if ("MC|TrList".equals ( packetIn.getChannelName ( ) )) {
            ...
            if (itemstack != null && itemstack.getItem ( ) == Items.written_book) {
                this.gameController
                        .displayGuiScreen ( new GuiScreenBook ( Minecraft.thePlayer , itemstack , false ) );
            }
        }
    }
